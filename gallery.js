<script>
  ...
  async function fetchFromApi() {
    try {
      ...
      return JSON.parse(localStorage.getItem('artGridSubmissions') || '[]');
    }
  }
  ...
  // Clear all: calls DELETE endpoint with admin key (optional)
  clearAllBtn.addEventListener('click', async () => {
    if (!confirm('Clear all submissions? This cannot be undone.')) return;
    // Only call API if admin key supplied in prompt (for convenience)
